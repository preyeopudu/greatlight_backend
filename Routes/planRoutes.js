...
router.post('/:user/plan/:plan', (req, res) => {
    ...
    let plan
    if(planName=="emerald"){
        plan={name:"EMERALD",cost:15000,daily:800,matureDate:Date.now()+29*24*60*60*1000}
        bonus=750
    }
    else if(planName=='ruby'){
        plan={name:"RUBY",cost:30000,daily:1600,matureDate:Date.now()+29*24*60*60*1000}
        bonus=1500
    }
    else if(planName=="sapphire"){
        plan={name:"SAPPHIRE",cost:60000,daily:3200,matureDate:Date.now()+29*24*60*60*1000}
        bonus=3000
    }
        ...
        plan={name:"CHRYSOLITE",cost:240000,daily:12800,matureDate:Date.now()+29*24*60*60*1000}
        bonus=12000
    }
    else if(planName=="agate"){
        plan={name:"AGATE",cost:480000,daily:25600,matureDate:Date.now()+29*24*60*60*1000}
        bonus=24000
    }
    else if(planName=="amethyst"){
        plan={name:"AMETHYST",cost:960000,daily:51200,matureDate:Date.now()+29*24*60*60*1000}
        bonus=48000
    }
