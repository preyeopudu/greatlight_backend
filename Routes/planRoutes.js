...
router.post('/:user/plan/:plan', (req, res) => {
    ...
    let plan
    if(planName=="emerald"){
        plan={name:"EMERALD",cost:15000,daily:650,matureDate:Date.now()+30*24*60*60*1000}
        bonus=750
    }
    else if(planName=='ruby'){
        plan={name:"RUBY",cost:30000,daily:1300,matureDate:Date.now()+30*24*60*60*1000}
        bonus=1500
    }
    else if(planName=="sapphire"){
        plan={name:"SAPPHIRE",cost:60000,daily:2600,matureDate:Date.now()+30*24*60*60*1000}
        bonus=3000
    }
    else if(planName=="onyx"){
        plan={name:"ONYX",cost:120000,daily:5200,matureDate:Date.now()+30*24*60*60*1000}
        bonus=6000
    }
    else if(planName=="chrysolite"){

        plan={name:"CHRYSOLITE",cost:240000,daily:10400,matureDate:Date.now()+30*24*60*60*1000}
        bonus=12000
    }
    else if(planName=="agate"){
        plan={name:"AGATE",cost:480000,daily:20800,matureDate:Date.now()+30*24*60*60*1000}
        bonus=24000
    }
    else if(planName=="amethyst"){
        plan={name:"AMETHYST",cost:960000,daily:41600,matureDate:Date.now()+30*24*60*60*1000}
        bonus=48000
    }
