  ...
  User.findOne({ username: req.params.user }, (err, user) => {
    if (err || user == null) {
      ...
      User.findOne({ username: req.body.user }, (err, recipient) => {
        if (err || recipient == null || req.params.user == req.body.user) {
          res.json({ userFalse: true });
        } else {
          if (user.deposit >= amount || user.Amount >= amount) {
            if (user.deposit >= amount) {
              Receipt.create(
                { text: `${user.name} transferred ${amount} NGN to you.` },
                (err, recipientReceipt) => {
                  if (user.deposit >= amount) {
                    Receipt.create(
                      {
                        text: `you transferred ${amount} NGN to ${recipient.name}.`,
                      },
                      (err, userReceipt) => {
                        ...
                        user.save(() => {
                          ...
                          recipient.deposit =
                            Number(recipient.deposit) + Number(amount);
                          recipient.save();
                        });
                      }
                    );
